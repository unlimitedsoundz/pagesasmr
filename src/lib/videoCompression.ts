 ...
 * and regulating video bitrate to drastically reduce file sizes during upload and download.
 */
export async function compressVideoFile(
  ...
  video.muted = false; // We use AudioContext to silently capture without speaker output
  ...
  try {
    ...
    // 5. Connect AudioContext for silent, pristine audio capture
    ...
    if (AudioContextClass) {
      try {
        ...
        // Note: sourceNode is NOT connected to audioCtx.destination,
        // which means the user's speaker remains silent while compressing!
    ...
    const videoBitrate = options.videoBitrate || 2_000_000; // ~2.0 Mbps
    const audioBitrate = options.audioBitrate || 128_000; // 128 kbps
    ...
    // Resume AudioContext if suspended (required on mobile Chrome / iOS)
    ...
    try {
      ...
      console.warn('Playback without mute failed on mobile, retrying with muted:', playErr);
      video.muted = true;
      await video.play();
    }
    renderLoop();
    ...
    // Generate output file extension based on mimeType
